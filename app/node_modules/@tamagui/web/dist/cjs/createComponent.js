...
function createComponent(staticConfig) {
  ...
  const component = (0, import_react.forwardRef)((propsIn, forwardedRef) => {
    ...
    const isHydrated = config != null && config.disableSSR ? !0 : (0, import_use_did_finish_ssr.useDidFinishSSR)();
    ...
    const initialState = willBeAnimated ? supportsCSSVars ? import_defaultComponentState.defaultComponentStateShouldEnter : import_defaultComponentState.defaultComponentState : import_defaultComponentState.defaultComponentStateMounted, states = (0, import_react.useState)(initialState), state = propsIn.forceStyle ? { ...states[0], [propsIn.forceStyle]: !0 } : states[0], setState = states[1];
    ...
    const componentClassName = props.asChild ? "" : props.componentName ? `is_${props.componentName}` : defaultComponentClassName, hasTextAncestor = !!(import_constants.isWeb && isText && componentContext.inText), isDisabled = props.disabled ?? ((_c = props.accessibilityState) == null ? void 0 : _c.disabled);
    ...
    const isAnimatedReactNative = hasAnimationProp && (animationsConfig == null ? void 0 : animationsConfig.isReactNative), isReactNative = !!(staticConfig.isReactNative || isAnimatedReactNative), shouldAvoidClasses = !!(!import_constants.isWeb || isAnimated || !staticConfig.acceptsClassName || propsIn.disableClassName), shouldForcePseudo = !!propsIn.forceStyle, noClassNames = shouldAvoidClasses || shouldForcePseudo, disableThemeProp = props["data-disable-theme"], disableTheme = disableThemeProp && !willBeAnimated || isHOC;
    ...
    typeof stateRef.current.isListeningToTheme == "boolean" && (themeStateProps.shouldUpdate = () => stateRef.current.isListeningToTheme);
    const isExiting = !state.unmounted && (presence == null ? void 0 : presence[0]) === !1;
    if (process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile") {
      const name = `${componentName || (Component == null ? void 0 : Component.displayName) || (Component == null ? void 0 : Component.name) || "[Unnamed Component]"}`, type = isAnimatedReactNative ? "(animated)" : isReactNative ? "(rnw)" : "", dataIs = propsIn["data-is"] || "", banner = `${name}${dataIs ? ` ${dataIs}` : ""} ${type} id ${internalID}`;
      console.group(
...
If you meant to do this, you can disable this warning - either change untilMeasured and group at the same time, or do group={conditional ? 'name' : undefined}`
    ...
    const disabled = ((_f = props.accessibilityState) == null ? void 0 : _f.disabled) || // @ts-expect-error (comes from core)
    ...
    )), !isReactNative && !willBeAnimated && !asChild ? viewProps = (_h = (_g = import_setupHooks.hooks).usePropsTransform) == null ? void 0 : _h.call(_g, elementType, nonTamaguiProps, hostRef) : viewProps = nonTamaguiProps;
...
` && console.error(
    ...
    let fontFamily = isText ? splitStyles.fontFamily || ((_i = staticConfig.defaultProps) == null ? void 0 : _i.fontFamily) : null;
    ...
    const events = shouldAttach && !isDisabled && !props.asChild ? {
      ...
      onPress: attachPress ? (e) => {
        unPress(), import_constants.isWeb && (onClick == null || onClick(e)), onPress == null || onPress(e), onLongPress == null || onLongPress(e);
      } : void 0,
      ...attachFocus && {
        onFocus: (e) => {
          setStateShallow({
    ...
    const ResetPresence = (_l = config == null ? void 0 : config.animations) == null ? void 0 : _l.ResetPresence;
    if (willBeAnimated && presence && ResetPresence && (content = /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ResetPresence, { children: content })), asChild) {
    ...
    import_setupHooks.hooks.useChildren && (useChildrenResult = import_setupHooks.hooks.useChildren(
      ...
      events,
      staticConfig
    )), useChildrenResult ? content = useChildrenResult : content = (0, import_react.createElement)(elementType, viewProps, content), process.env.NODE_ENV === "development" && time && time`create-element`;
