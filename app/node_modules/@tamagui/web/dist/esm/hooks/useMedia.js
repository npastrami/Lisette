...
function useMedia(uid, componentContext) {
  const internal = useRef(), initialState = ((componentContext ? getDisableSSR(componentContext) : useDisableSSR()) || !isWeb ? mediaState : initState) || {}, state = useSyncExternalStore(
    subscribe,
    () => {
      if (!internal.current)
        return initialState;
