...
function createComponent(staticConfig) {
  ...
  process.env.NODE_ENV === "development" && staticConfig.defaultProps?.debug && process.env.IS_STATIC !== "is_static" && log(`\u{1F41B} [${staticConfig.componentName || "Component"}]`, {
    staticConfig,
    defaultProps,
  ...
  const component = forwardRef((propsIn, forwardedRef) => {
    ...
    if (process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile") {
      const name = `${componentName || Component?.displayName || Component?.name || "[Unnamed Component]"}`, type = isAnimatedReactNative ? "(animated)" : isReactNative ? "(rnw)" : "", dataIs = propsIn["data-is"] || "", banner = `${name}${dataIs ? ` ${dataIs}` : ""} ${type} id ${internalID}`;
      console.group(
...
` && console.error(
    ...
    willBeAnimated && presence && ResetPresence && (content = /* @__PURE__ */ jsx(ResetPresence, { children: content })), asChild && (elementType = Slot, Object.assign(viewProps, { onPress, onLongPress })), process.env.NODE_ENV === "development" && time && time`spaced-as-child`;
    ...
    hooks.useChildren && (useChildrenResult = hooks.useChildren(
      ...
      events,
      staticConfig
    )), useChildrenResult ? content = useChildrenResult : content = createElement(elementType, viewProps, content), process.env.NODE_ENV === "development" && time && time`create-element`;
    ...
    if (process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile") {
      ...
      try {
        log("viewProps", viewProps), log("viewPropsOrder", Object.keys(viewProps));
        for (const key in viewProps)
          log(" - ", key, viewProps[key]);
        log("children", content), typeof window < "u" && log({
          viewProps,
          state,
          styleProps,
          themeState,
