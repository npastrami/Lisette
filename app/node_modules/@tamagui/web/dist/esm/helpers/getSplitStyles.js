...
const PROP_SPLIT = "-", getSplitStyles = (props, staticConfig, theme, themeName, componentState, styleProps, parentSplitStyles, context, elementType, debug) => {
  ...
  if (process.env.NODE_ENV === "development" && debug && debug !== "profile" && isClient && (console.groupCollapsed("getSplitStyles (collapsed)"), log({
    props,
    staticConfig,
    shouldDoClasses,
    styleProps,
  ...
  for (const keyOg in props) {
    ...
    if (styleState.curProps[keyInit] = valInit, valInitType === "string" && valInit[0] === "_" && (keyInit in validStyleProps || keyInit.includes("-"))) {
      process.env.NODE_ENV === "development" && debug && log(`Adding compiled style ${keyInit}: ${valInit}`), shouldDoClasses ? (classNames[keyInit] = valInit, delete style[keyInit]) : (style[keyInit] = reverseMapClassNameToValue(keyInit, valInit), delete className[keyInit]);
      continue;
    }
    if (keyInit === "dataSet") {
    ...
    if (!styleProps.noExpand) {
      ...
      if (valInit && valInit[0] === "_") {
        ...
        if (isValidClassName || isMediaOrPseudo2) {
          process.env.NODE_ENV === "development" && debug && log("tamagui classname prop", keyInit, valInit), shouldDoClasses ? (mergeClassName(transforms, classNames, keyInit, valInit, isMediaOrPseudo2), delete style[keyInit]) : (style[keyInit] = reverseMapClassNameToValue(keyInit, valInit), delete className[keyInit]);
          continue;
        }
      }
    }
...
current`, {
      ...
      for (const [key, val] of expanded) {
        ...
        if (isPseudo) {
          ...
          if (shouldDoClasses && !isEnter && !isExit) {
            ...
            isWeb && !isClient && isEnter && (isDisabled = !1), process.env.NODE_ENV === "development" && debug === "verbose" && (console.groupCollapsed("pseudo", key, { isDisabled }), log(pseudoStyleObject, {
              isDisabled,
              descriptorKey,
              descriptor,
              pseudoState,
  ...
  if (props.style && mergeStylePropIntoStyle(styleState, props.style), !(styleProps.noNormalize === !1) && (fixStyles(style), isWeb && !staticConfig.isReactNative && styleToCSS(style), styleState.transforms && (Object.entries(styleState.transforms).sort(([a], [b]) => a.localeCompare(b)).forEach(([key, val]) => {
