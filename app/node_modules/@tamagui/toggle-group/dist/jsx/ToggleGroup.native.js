  ...
  React.forwardRef(
    (props, forwardedRef) => {
      const valueContext = useToggleGroupValueContext(props.__scopeToggleGroup), context = useToggleGroupContext(props.__scopeToggleGroup), pressed = valueContext == null ? void 0 : valueContext.value.includes(props.value), disabled = context.disabled || props.disabled || !1, groupItemProps = useGroupItem({ disabled }), size = props.size ?? context.size, sizeProps = props.unstyled ? {} : {
        ...
        {...groupItemProps}
      />;
      return <ToggleGroupItemProvider scope={props.__scopeToggleGroup}>{context.rovingFocus ? <RovingFocusGroup.Item
        asChild="except-style"
        __scopeRovingFocusGroup={props.__scopeToggleGroup || TOGGLE_GROUP_CONTEXT}
        focusable={!disabled}
        active={pressed}
      >{inner}</RovingFocusGroup.Item> : inner}</ToggleGroupItemProvider>;
    }
  )
);
